 ...
 */
@Controller
@RequestMapping("/system/firstcheck")
public class CommentFirstCheckController extends BaseController
{
	...
	@RequiresPermissions("system:firstCheck:view")
	@GetMapping()
	public String activityComment()
	{
	    return prefix + "/firstcheck";
	}
	
	/**
	 * 查询留言列表
	 */
	@RequiresPermissions("system:firstCheck:list")
	@PostMapping("/list")
	@ResponseBody
	public TableDataInfo list(ActivityComment activityComment)
	{
		startPage();
        List<ActivityComment> list = activityCommentService.selectFirstCheckCommentList(activityComment);
	 ...
	 * 一级审核通过
	 */
	@Log(title = "一级审核", businessType = BusinessType.UPDATE)
	@RequiresPermissions("system:firstCheck:check")
	@PostMapping( "/check")
	@ResponseBody
